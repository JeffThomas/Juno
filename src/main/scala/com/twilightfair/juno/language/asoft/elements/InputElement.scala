 ...
 */
class InputElement(config: PrefixConfig)
  ...
  def execute(proc: Process): Process = {
    val inputEl = proc.heap.headOption
    ...
    inputEl.map { inputs =>
      ...
      val inputList1 = inputs.value match {
        case rl: List[Any] => rl
        case _ => List(inputs)
      }
      val label = inputList1(0).asInstanceOf[Value] match {
        case label if label.ref.isEmpty => label.value.asInstanceOf[String]
        case _ => ""
      }
      inputList1.map { readInto =>
  ...
  lazy val graphName = s"Input${InputElement.getCount}"
...
object InputElement extends ElementFactory {
  def apply(config: ElementConfig) = {
    val element = new InputElement(config.asInstanceOf[PrefixConfig])
    Right(Some(element))
  }
